...
def parseArgs():
    ...
    parser.add_argument("--sort-by", help="How to sort rows of the output matrix. Options: input, mean_desc, mean_asc, max_desc, max_asc, metadata:<column>.",
                        default="mean_desc")
    parser.add_argument("--dataType", help="Data type: enrichment (default), wgbs01 (values 0-1), wgbs0100 (values 0-100).",
                        default="enrichment", choices=["enrichment", "wgbs01", "wgbs0100"])
    ...
    if args.dataType == "enrichment":
        if args.bin_size is None and args.number_of_bins is None:
            parser.error("One of --bin-size or --number-of-bins must be provided for enrichment data.")
...
def extract_wgbs_values(path: str, chrom: str, start: int, end: int) -> List[float]:
    with pyBigWig.open(path) as bw:
        vals = bw.values(chrom, start, end, numpy=False)
...
def main():
    ...
    with open(args.output, "w") as handle:
        ...
        handle.write(f"#dataType\t{args.dataType}\n")
